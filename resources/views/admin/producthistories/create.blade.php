...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.producthistory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.producthistories.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('software') ? 'is-invalid' : '' }}" type="text" name="software" id="software" value="{{ old('software', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('item_names') ? 'is-invalid' : '' }}" name="item_names[]" id="item_names" multiple>
                    @foreach($item_names as $id => $item_name)
                        <option value="{{ $id }}" {{ in_array($id, old('item_names', [])) ? 'selected' : '' }}>{{ $item_name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('item_name_1s') ? 'is-invalid' : '' }}" name="item_name_1s[]" id="item_name_1s" multiple>
                    @foreach($item_name_1s as $id => $item_name_1)
                        <option value="{{ $id }}" {{ in_array($id, old('item_name_1s', [])) ? 'selected' : '' }}>{{ $item_name_1 }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('item_name_2s') ? 'is-invalid' : '' }}" name="item_name_2s[]" id="item_name_2s" multiple>
                    @foreach($item_name_2s as $id => $item_name_2)
                        <option value="{{ $id }}" {{ in_array($id, old('item_name_2s', [])) ? 'selected' : '' }}>{{ $item_name_2 }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('item_name_3s') ? 'is-invalid' : '' }}" name="item_name_3s[]" id="item_name_3s" multiple>
                    @foreach($item_name_3s as $id => $item_name_3)
                        <option value="{{ $id }}" {{ in_array($id, old('item_name_3s', [])) ? 'selected' : '' }}>{{ $item_name_3 }}</option>
                    @endforeach
                </select>
